...
def send_channel_post(chat_id, aid, title, genre, status, poster):
    try:
        ...
        if poster:
            # Video yoki rasm ekanini aniqlash
            try:
                bot.send_video(CHANNEL, poster, caption=text, reply_markup=kb, parse_mode='HTML')
            except:
                try:
...
def admin_panel(msg):
    ...
    kb.add(telebot.types.InlineKeyboardButton('📢 Kanalga post', callback_data='post_anime'))
    kb.add(telebot.types.InlineKeyboardButton('🔀 Animelarni birlashtirish', callback_data='merge_anime'))
    bot.send_message(msg.chat.id,
                     f'👑 <b>Admin Panel</b>\n\n👥 Foydalanuvchilar: {u}\n🎬 Animelar: {a}',
                     reply_markup=kb, parse_mode='HTML')
...
@bot.callback_query_handler(func=lambda c: c.data == 'post_anime')
def post_anime_cb(call):
    if call.from_user.id not in ADMIN_IDS:
        return
    user_states[call.from_user.id] = {'step': 'post_id'}
    bot.send_message(call.message.chat.id, '📢 Kanalga post qilmoqchi anime ID sini kiriting:')
    bot.answer_callback_query(call.id)


@bot.callback_query_handler(func=lambda c: c.data == 'merge_anime')
def merge_anime_cb(call):
    if call.from_user.id not in ADMIN_IDS:
        return
    bot.send_message(call.message.chat.id,
                     '🔀 Animelarni birlashtirish\n\n'
                     'Buyruq: /merge eski_id yangi_id fasl\n\n'
                     'Misol: /merge 2 1 2\n'
                     'ID=2 dagi animelarni ID=1 ga 2-fasl sifatida qoshadi',
                     parse_mode='HTML')
    bot.answer_callback_query(call.id)


@bot.message_handler(func=lambda m: m.from_user.id in user_states and user_states[m.from_user.id].get('step') == 'post_id')
def get_post_id(msg):
    try:
...
def handle_text(msg):
    ...
    if text == '📞 Murojaat':
        bot.send_message(msg.chat.id,
                         '📞 <b>Murojaat</b>\n\n'
                         '👤 Admin: @g1yu_tamioka\n'
                         '📢 Kanal: https://t.me/animedunyosii_uzb',
                         parse_mode='HTML')
        return
...
def merge_cmd(msg):
    ...
    if len(args) < 4:
        bot.send_message(msg.chat.id,
                         '📝 Ishlatish: /merge <eski_id> <yangi_id> <fasl>\n'
                         'Masalan: /merge 2 1 2\n'
                         '(2-IDdagi animelarni 1-IDga 2-fasl sifatida qo\'shadi)')
        return
    try:
        old_id = int(args[1])
        new_id = int(args[2])
        new_season = int(args[3])

        conn = get_conn()
        c = conn.cursor()

        c.execute('SELECT title FROM anime WHERE anime_id=%s LIMIT 1', (old_id,))
        old_row = c.fetchone()
        c.execute('SELECT title FROM anime WHERE anime_id=%s LIMIT 1', (new_id,))
        new_row = c.fetchone()

        if not old_row:
            bot.send_message(msg.chat.id, f'❌ {old_id} IDda anime topilmadi!')
            conn.close()
            return
        if not new_row:
            bot.send_message(msg.chat.id, f'❌ {new_id} IDda anime topilmadi!')
            conn.close()
            return

        c.execute('SELECT episode, video FROM anime WHERE anime_id=%s ORDER BY episode', (old_id,))
        eps = c.fetchall()

        c.execute('SELECT title, genre, category, status FROM anime WHERE anime_id=%s LIMIT 1', (new_id,))
        new_info = c.fetchone()
        new_title = new_info[0]
        new_genre = new_info[1]
        new_category = new_info[2]
        new_status = new_info[3]

        moved = 0
        for ep, vid in eps:
            try:
                c.execute(
                    'INSERT INTO anime (anime_id, title, genre, season, episode, video, category, status) '
                    'VALUES (%s, %s, %s, %s, %s, %s, %s, %s) '
                    'ON CONFLICT (anime_id, season, episode) DO UPDATE SET video=EXCLUDED.video',
                    (new_id, new_title, new_genre, new_season, ep, vid, new_category, new_status))
                moved += 1
            except:
                pass
        ...
        sl = 'OVA' if new_season == 0 else f'{new_season}-fasl'
        bot.send_message(msg.chat.id,
                         f'✅ <b>{old_row[0]}</b> dan {moved} ta qism\n'
                         f'<b>{new_row[0]}</b> ga {sl} sifatida ko\'chirildi!\n\n'
                         f'⚠️ Eski animeni o\'chirish uchun: /del {old_id}',
                         parse_mode='HTML')
    except Exception as e:
        bot.send_message(msg.chat.id, f'❌ Xatolik: {e}')
...
def del_cmd(msg):
    ...
    args = msg.text.split()
    if len(args) < 2:
        bot.send_message(msg.chat.id, '📝 Ishlatish: /del <anime_id>\nMasalan: /del 2')
        return
    try:
        aid = int(args[1])
        conn = get_conn()
        ...
        if row:
            c.execute('DELETE FROM anime WHERE anime_id=%s', (aid,))
            conn.commit()
            bot.send_message(msg.chat.id, f'✅ <b>{row[0]}</b> (ID:{aid}) ochirildi!', parse_mode='HTML')
        else:
            bot.send_message(msg.chat.id, '❌ Bu ID da anime topilmadi!')
        conn.close()
    except Exception as e:
